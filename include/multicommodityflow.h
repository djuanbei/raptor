 ...
 private:
  ...
  inline void differPhi() {
    ...
    for (i = 0; i < link_num; i++) {
      size_t index=4*i;
      allAllow[index] = alpha / allAllow[index + ORIGINAL_CAPACITY] * allAllow[index+ ORIGINAL_PRICE ]
                        exp(alpha * allAllow [index+ CURRENT_ALLOW] / allAllow[index + ORIGINAL_CAPACITY]);
    }
  }
