...
class flow_task {
 ...
 public:
  flow_task(const size_t s, const size_t e, const double d)
      : isSingle(false), src(s), snk(e), demand(d) {}
  void setIsSingle(bool b) { isSingle = b; }
  bool getIsSingle(void) { return isSingle; }
...
class flow {
 ...
 public:
  ...
  vector<size_t> getPaths(void) { return ver_links; }
 ...
 private:
  ...
  inline void differPhi() {
    ...
    for (i = 0; i < link_num; i++) {
      allAllow[3 * i] = alpha / allAllow[3 * i + 1] *
                        exp(alpha * allAllow[3 * i + 2] / allAllow[3 * i + 1]);
    }
  }
