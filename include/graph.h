 ...
 class compressed_sparse_row_graph {
  ...
  public:
   ...
   void initial(vector<E> &srcs, vector<E> &snks, vector<W> &weights, bool save=true) {
   ...
   bool compute_shortest_path_dijkstra(const E src, const E snk,
     ...
     while (!Q.empty()) {
       ...
       if (current == snk) {
         while (current != src) {
           path.push_back(link_map[preLink[current]]);
   ...
   bool compute_kshortest_path_dijkstra(const int K, const E src, const E snk,
           ...
           for( k=i; k< K; k++ ){
             ...
             if( weight < dis[  next ]  ) {
               if( preLink[ next ].link < link_num ){
                 for( h=K-1; h>k; h-- ){
                   preLink[ K*neighbour.snk+h ] =preLink[  K*neighbour.snk+h-1 ];
                 }
               }
     ...
     for( i=0; i< K; i++ ){
       ...
       while (num < vertex_num + 2 && current != src) {
         link =  current * K + pid;
         path.push_back(link_map[preLink[link].link]);
