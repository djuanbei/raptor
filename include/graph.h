...
template <class T> struct LESSOR {
...
class compressed_sparse_row_graph {
 ...
 public:
  ...
  template <typename T> vertex_map<T> get_vertex_map() {
    return vertex_map<T>(*this);
  }

  compressed_sparse_row_graph(const size_t k = 1)
      : save_path(true), infi_value(numeric_limits<W>::max() / 10e10),
        ksp_k(k), vertex_num(0), link_num(0) {
    assert(k > 0);
  }
  void clear() {
  ...
  E  getAdj(int v, int i  )const{
    return in2outLink_map[outIndex[ v ]+i];
  }
  ...
  void suurballe_shortest(const E src, const E snk, vector<E> &path1,
    ...
    while (!Q.empty()) {
      ...
      // reverse all directtion of link in path1 and  cut all link direct
      // to src
...
class undir_graph {
 ...
 public:
  ...
  inline int getOutDegree(E vertex) const {
    if (vertex >= vertex_num)
      return 0;
    return outIndex[vertex + 1] - outIndex[vertex]+inIndex[ vertex + 1 ]-inIndex[ vertex ];
  }
