...
/* True if a == 0 */
#define IS_ZERO(a) (SCALAR_IS_ZERO((a).Real) && SCALAR_IS_ZERO((a).Imag))

/* -------------------------------------------------------------------------- */

/* True if a is NaN */
#define IS_NAN(a) (SCALAR_IS_NAN((a).Real) || SCALAR_IS_NAN((a).Imag))
...
#define DIV(c, a, b)                                                           \
  {                                                                            \
    (void)utDivideComplex((a).Real, (a).Imag, (b).Real, (b).Imag, &((c).Real), \
                          &((c).Imag));                                        \
  }
#else
