 ...
 *
 *
 * @param first Forward iterators to the initial and final positions of a sorted
 * (or properly partitioned) sequence. The range used is [first,last), which
 ...
 */
template <typename T>
int binfind(typename vector<T>::const_iterator first,
            typename vector<T>::const_iterator last, T value) {
...
template <typename T>
int binfind(typename vector<T>::iterator first,
            typename vector<T>::iterator last, T value) {
